...
class DataFrame {
   ...
   */
  filter (op) {
    util.check(typeof op === 'function',
               `filter expression is not a function`)
    const newData = this.data.filter(op)
   ...
   * @param {string} operations A list of [function, columnName] operation specs.
   * @returns A new dataframe.
   */
  summarize (operations) {
    // Check operation specs.
    util.check(Array.isArray(operations) &&
               operations.every(item => (Array.isArray(item) && (item.length === 2))),
               `Operations must be array of pairs`)
    operations.forEach(([func, sourceCol]) =>
                       util.check(sourceCol, `empty column(s) in summarize`))
    const colNames = operations.map(([func, sourceCol]) => sourceCol)
    util.check(this.hasColumns(colNames),
               `unknown column(s) in summarize`)
    util.check(operations.every(([func, sourceCol]) => (typeof func === 'function')),
               `operation functions must be functions`)
    util.check(operations.every(([func, sourceCol]) => (Summarize.SUFFIX in func)),
               `operation functions must have ${Summarize.SUFFIX} property`)

    // Summarize operation by operation.
    const newData = this.data.map(row => { return {...row} })
    const newColumnNames = []
    operations.forEach(([func, sourceCol]) => {
      const destCol = `${sourceCol}_${func[Summarize.SUFFIX]}`
      newColumnNames.push(destCol)
      this._summarizeColumn(newData, func, sourceCol, destCol)
    })
  ...
  _findUnique (seen, newData, row, i, columns) {
    const thisValue = Op.get(row, i, columns[0])
  ...
  //
  _summarizeColumn (data, func, sourceCol, destCol) {
    ...
    for (let groupId of groups.keys()) {
      const result = func(groups.get(groupId), sourceCol)
